...
# SenseFuzeAI - Keystroke Model Training and Selection Pipeline
#
# Trains and compares multiple classifiers for the keystroke modality.
#
...
#
# Input:
#   emosurv_processed/combined_behaviour_samples.csv
#
# Output:
#   model_artifacts/keystroke_model.joblib
#   model_artifacts/keystroke_model_meta.json
#   model_artifacts/keystroke_training_report.txt
#   model_artifacts/visual_reports/keystroke/*.png
#   model_artifacts/visual_reports/keystroke/*.csv
...
DATA_PATH = BASE_DIR / "emosurv_processed" / "combined_behaviour_samples.csv"

MODEL_DIR = BASE_DIR / "model_artifacts"
MODEL_DIR.mkdir(parents=True, exist_ok=True)

MODEL_PATH = MODEL_DIR / "keystroke_model.joblib"
META_PATH = MODEL_DIR / "keystroke_model_meta.json"
REPORT_PATH = MODEL_DIR / "keystroke_training_report.txt"

VISUAL_REPORT_DIR = MODEL_DIR / "visual_reports"

TARGET = "behaviour_state"
...
def validate_dataset(df: pd.DataFrame) -> pd.DataFrame:
    """
    Validate and clean the keystroke dataset.
    """
    missing = [
    ...
    if missing:
        raise ValueError(
            ...
            f"{missing}\n\n"
            "Run the latest data_cleaning.py first."
        )
    ...
    if df.empty:
        raise ValueError("No valid behavioural samples found.")
    ...
    if missing_classes:
        raise ValueError(f"Missing behavioural classes: {missing_classes}")
...
def build_candidate_pipelines() -> dict[str, Pipeline]:
    """
    Build candidate classifiers for keystroke model selection.
    """
    return {
        "svm_rbf": Pipeline(
            steps=[
...
def evaluate_candidates(
    ...
    if best_name is None or best_model is None:
        raise RuntimeError("No candidate keystroke model was successfully trained.")
...
def save_metadata(
    ...
    """
    Save machine-readable metadata for the selected keystroke model.
    """
    metadata = {
        "model_name": f"keystroke_{best_name}_typing_dynamics",
        ...
        "method": f"keystroke_dynamics_plus_{best_name}",
        "timing_unit": "milliseconds",
        "typing_speed_unit": "keys_per_millisecond",
        "label_type": "heuristic_pseudo_label",
        "test_size": TEST_SIZE,
        "random_state": RANDOM_STATE,
        ...
        "behaviour_mapping": {
            "focused": "High speed, consistent rhythm, low errors",
            "distracted": "Irregular typing, long pauses, inconsistent speed",
            "fatigued": "Lower speed, higher IKI, backspaces, mental blocks",
            "overloaded": "Erratic fits-and-starts rhythm, high error proxy",
        },
        "methodological_note": (
            "This model estimates behavioural typing states from keystroke "
            "dynamics. Labels are heuristic pseudo-labels and should be "
            "interpreted as behavioural typing proxies, not direct psychological "
            "ground truth. Multiple classifiers were compared, and the saved "
            "keystroke_model.joblib is the best candidate by macro F1-score, "
            "with accuracy used as the tie-breaker. Visual report artefacts "
            "are generated for use in the final report and prototype video."
        ),
    }
...
def save_training_report(
    ...
    """
    Save a human-readable keystroke training report.
    """
    ...
    with open(REPORT_PATH, "w", encoding="utf-8") as f:
        f.write("Keystroke Model Training and Selection Report\n")
        f.write("=============================================\n\n")
        ...
        f.write("\nMethodological note:\n")
        f.write(
            "The keystroke model selection process compares several multi-class "
            "classification algorithms over typing-dynamics features. The "
            "selected model is chosen using macro F1-score as the primary "
            "criterion because all four behaviour classes are important. "
            "Accuracy is used only as a tie-breaker. The labels are heuristic "
            "pseudo-labels and should be interpreted as behavioural typing "
            "proxies rather than direct psychological ground truth."
        )
...
def main() -> None:
    print("==============================================")
    print("SenseFuzeAI Keystroke Model Training Pipeline")
    print("==============================================")
    ...
    print(f"Loading keystroke dataset: {DATA_PATH}")
    ...
    print("\nTraining and comparing keystroke classifiers...")
    ...
    print("Saving selected keystroke model artifact...")
