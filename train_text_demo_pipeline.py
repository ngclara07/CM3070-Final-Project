...
DATA_PATH = Path("data/processed/text_features.csv")
OUTPUT_DIR = Path("models/text_demo")
...
def load_dataset():
    if not DATA_PATH.exists():
        raise FileNotFoundError(
            f"Text feature file not found: {DATA_PATH}\n"
            "Run extract_text_features.py first."
        )
    ...
    if not feature_cols:
        raise ValueError("No numeric text embedding features found.")
...
def main():
    ...
    print(f"\nBest text model: {best_model_name}")

    best_pipeline.fit(X, y)

    joblib.dump(best_pipeline, OUTPUT_DIR / "text_pipeline.joblib")
    ...
    print("\nText demo pipeline saved successfully.")
    print(f"Selected model: {best_model_name}")
    print(f"Model path: {OUTPUT_DIR / 'text_pipeline.joblib'}")
