...
#   python download_yamnet.py --force
#   python download_yamnet.py --verify-only
#
...
def resolve_or_download_tfhub_model(tfhub_url: str) -> Path:
    ...
    If the TFHub cache is corrupted, delete it once and retry.
    """
    import tensorflow_hub as hub

    tfhub_cache_dir = Path(os.environ.get(
        "TFHUB_CACHE_DIR",
        Path(os.environ["LOCALAPPDATA"]) / "Temp" / "tfhub_modules"
    ))
    ...
    def resolve_once() -> Path:
        ...
        resolved = Path(hub.resolve(tfhub_url)).resolve()
    ...
    resolved_path = resolve_once()

    if is_valid_saved_model_dir(resolved_path):
        return resolved_path

    print("Resolved TFHub model is invalid.")
    print("Deleting corrupted TensorFlow Hub cache and retrying once...\n")
    print(f"Cache directory:\n{tfhub_cache_dir}\n")

    remove_directory(tfhub_cache_dir)

    resolved_path = resolve_once()

    if not is_valid_saved_model_dir(resolved_path):
        fail(
            "Resolved TensorFlow Hub path is still not a valid SavedModel directory.\n\n"
            f"Resolved path:\n{resolved_path}\n\n"
            "Possible causes:\n"
            "1. Internet download was interrupted.\n"
            "2. Antivirus/OneDrive blocked extraction.\n"
            "3. TensorFlow Hub package version issue.\n\n"
            "Try:\n"
            "  pip install --upgrade tensorflow tensorflow-hub tf-keras\n"
            "  python download_yamnet.py --force"
        )
