...
DATA_PATH = Path("data/processed/image_features.csv")
OUTPUT_DIR = Path("models/image_demo")
...
def load_dataset():
    if not DATA_PATH.exists():
        raise FileNotFoundError(
            f"Image feature file not found: {DATA_PATH}\n"
            "Run extract_image_features.py first."
        )
    ...
    if not feature_cols:
        raise ValueError("No numeric image features found.")
...
def main():
    ...
    print(f"\nBest image model: {best_model_name}")

    best_pipeline.fit(X, y)

    joblib.dump(best_pipeline, OUTPUT_DIR / "image_pipeline.joblib")
    ...
    print("\nImage demo pipeline saved successfully.")
    print(f"Selected model: {best_model_name}")
    print(f"Model path: {OUTPUT_DIR / 'image_pipeline.joblib'}")
